    ...
    async function askAIStudyAssistant() {
        ...
        try {
            ...
            if (media) {
                ...
                form.append("class_name", currentClass || "Class 11");
                response = await fetch("/api/ai/solve-media?response=json&v=20260926-final", {
                    method: "POST",
                    body: form,
                    cache: "no-store",
                ...
                response = await fetch("/api/ai/solve", {
                    ...
                    body: JSON.stringify({ question: question, class_name: currentClass || "Class 11" })
                });
            }
            ...
            try {
                ...
                throw new Error(
                    "The AI server returned HTML instead of JSON (HTTP " + response.status + "). " +
                    "This usually means the latest app.py has not finished deploying. " +
                    "Wait for Render to show Deploy live, then press Ctrl+Shift+R. " +
...
def api_ai_solve_media():
    try:
        ...
        response = jsonify({"success": True, "ai": ai})
