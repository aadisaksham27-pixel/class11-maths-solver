...
# ============================================================
# KSHITIJ AUTO-DETECT MATH ENGINE BRIDGE
# ============================================================
# The original Class 8-10 curriculum engines above are preserved.
# The unified engine is tried first so chapter selection is optional.
# If it cannot safely solve the question, the original school engines
# are used as a compatibility fallback.

try:
    from math_engine import solve_math as _unified_solve_math
except Exception:
    _unified_solve_math = None


_ORIGINAL_SOLVE_SCHOOL_MATH = solve_school_math


def solve_school_math(question, chapter="Auto Detect"):
    """
    Unified mathematics entry point.

    1. Automatically detects the topic.
    2. Uses the new general SymPy math engine first.
    3. Falls back to the existing Class 8-10 curriculum engines.
    4. Does not require the student to choose a chapter.
    """
    question = str(question or "").strip()

    if not question:
        return "Please enter a mathematics question."

    if _unified_solve_math is not None:
        try:
            result = _unified_solve_math(question)

            if result.get("success"):
                lines = [
                    f"### {result.get('topic', 'Mathematics')}",
                    "",
                ...
                lines.append("")
                lines.append(f"**Final answer:** {result.get('final_answer', '')}")
    ...
    # Existing Class 8-10 curriculum system remains as a fallback.
    try:
        return _ORIGINAL_SOLVE_SCHOOL_MATH(question, "Auto Detect")
    except Exception as error:
        return f"Unable to solve this question: {error}"
